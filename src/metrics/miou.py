...
def mIoU(pred_annons: List[Annotation], gt_annons: List[Annotation], score_available: bool = True) -> float:
    if score_available:
...
def mIoU_by_frame(pred_annons: List[Annotation], gt_annons: List[Annotation], score_available: bool = True) -> Dict[int, float]:
    if score_available:
        pred_annons = sorted(pred_annons, key=lambda x: x.score, reverse=True)
    else:
        pred_annons = np.random.shuffle(pred_annons)
